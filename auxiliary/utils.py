...
# ================================= Exponential Decay ============================== #
def exponential_decay(t, init=0.99, m=100, finish=0.01):
...
def exponential_decay_torch(t, init=0.99, m=100, finish=0.01):
    alpha = math.log(init / finish) / m
    l = - math.log(init) / alpha
    decay = math.exp(-alpha * (t + l))
    return decay


# ============================= Learning Rate Decay ================================ #
def adjust_learning_rate(optimizer, epoch, init_lr, lr_decay_factor=0.1, lr_decay_epoch=200):
...
# ================================ F-score with threshold ========================== #
def compute_F_score(distance1, distance2, threshold=1e-4):
    # print("[computing F-score]")
    recall = sum((distance2 < threshold).float()) / len(distance2)
    precision = sum((distance1 < threshold).float()) / len(distance1)
    fscore = 200 * recall * precision / (recall + precision)  # F-score(%) -- multiply 100 to compute percentage
    return fscore
